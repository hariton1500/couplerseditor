...
class _CableScreenState extends State<CableScreen> {
                ...
                ? TextButton.icon(
                    onPressed: () {
                      setState(() {
                        if (cables.any((c) =>
                            c.signature() ==
                            '${ends[0].signature()}:${ends[1].signature()}')) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
                                  'Cable already exists between ${ends[0].direction} and ${ends[1].direction}'),
                            ),
                          );
                        } else {
  ...
  Widget _buildList() {
    return Column(
      children: [
        couplers.isNotEmpty
            ? TranslateText('Couplers:', language: widget.lang)
            : Container(),
        ListView.builder(
          shrinkWrap: true,
        ...
        const Divider(),
        nodes.isNotEmpty
            ? TranslateText('Nodes:', language: widget.lang)
            : Container(),
        ListView.builder(
          shrinkWrap: true,
