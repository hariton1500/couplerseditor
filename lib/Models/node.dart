...
class Connection {
  ...
  Connection.fromJson(Map<String, dynamic> json) {
    for (var connectionMap in json['connections']) {
      ...
      (side1['side1'] as Map<String, dynamic>).containsKey('AD') ? connectionData = MapEntry(
          MapEntry(
          ...
          MapEntry(
              ...
              side2['side2']['port'] as int)) : connectionData = MapEntry(
          MapEntry(
  ...
  Map<String, dynamic> toJson() {
    var side1 = MapEntry(connectionData!.key.key is ActiveDevice ? ['AD', (connectionData!.key.key as ActiveDevice).ip] : ['CE', (connectionData!.key.key as CableEnd).direction], connectionData!.key.value);
    var side2 = MapEntry(connectionData!.value.key is ActiveDevice ? ['AD', (connectionData!.value.key as ActiveDevice).ip] : ['CE', (connectionData!.value.key as CableEnd).direction], connectionData!.value.value);
    return {'connectionData' : [{'side1' : side1.key, 'port': side1.value}, {'side2': side2.key, 'port': side2.value}]};
  }
}
